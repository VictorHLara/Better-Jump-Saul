...
class Inimigo_Atirador(Enemy):
    def __init__(self, x, y, largura, altura, sprites, projectile_image, projectile_speed):
        super().__init__(x, y, largura, altura, 0, sprites)
    ...
    def shoot(self):
        ...
        if current_time - self.last_shot >= self.shoot_cooldown:
            direction = -1 if self.direcao == "right" else 1
    ...
    def draw(self, tela, offset_x):
        super().draw(tela, offset_x) 
        for projectile in self.projectiles:
            projectile.draw(tela, offset_x)
